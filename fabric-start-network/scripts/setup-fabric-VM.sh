...
#    fi
    ...
    if [[ "$COUNT" -eq 1 ]]; then    # admin and user
...
if [[ "$1" == "registerOrdererID" ]]; then
    registerOrdererID $2 $3
elif [[ "$1" == "registerPeerID" ]]; then
    registerPeerID $2 $3
elif [[ "$1" == "setup" ]]; then
    setup
fi
